...
export default function ResultCard({ isDark }) {
  const [activeTab, setActiveTab] = useState("video");

  const formats =
    activeTab === "video"
      ? mockVideo.formats.filter((item) => item.type === "video")
      : mockVideo.formats.filter((item) => item.type === "audio");
    ...
    >
      <div className="grid gap-6 lg:grid-cols-[0.95fr_1.05fr]">
        <div className="relative overflow-hidden rounded-[26px]">
          ...
          <button className="absolute left-1/2 top-1/2 flex h-16 w-16 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full bg-black/55 text-white backdrop-blur-xl transition hover:scale-105">
        ...
        <div>
          ...
          <h2 className={`mt-4 text-2xl font-black leading-tight md:text-3xl ${titleClass}`}>
            {mockVideo.title}
          </h2>

          <p className={`mt-2 text-sm leading-6 ${textClass}`}>
            Choose your preferred format below. This is a UI preview with mock format data.
          </p>

          <div className={`mt-5 inline-flex rounded-2xl p-1 ${
            isDark ? "bg-black/25 ring-1 ring-white/10" : "bg-slate-100 ring-1 ring-slate-200"
          }`}>
            <button
